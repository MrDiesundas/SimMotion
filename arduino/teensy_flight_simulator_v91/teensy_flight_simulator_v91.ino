...
//const int MAX_FREQ_PITCH_ROLL = 70000;  // at 40000stp/rev
const int MAX_FREQ_PITCH_ROLL = 14000;  // at 4000stp/rev
...
void moveToTarget(char axis, float targetDeg, int targetSpeed) {
  ...
  if (axis == 'P' || axis == 'R') {
      ...
      // *** CRITICAL FIX ***
      analogWrite(stepPin, 0);          // stop any previous PWM
      analogWriteFrequency(stepPin, 1); // safe idle frequency
      delay(5);                         // allow driver to settle
